...
from utils import calculate_batch_correction_changes, calculate_biological_preservation
from utils import extract_metrics_with_calculations
import os
...
#         Directory containing kappa_mu_*_var_b_* subdirectories with JSON files.
...
#             try:
#                 kappa_mu = float(parts[2])  # kappa_mu_X_var_b_Y -> X
#                 var_b = float(parts[5])     # kappa_mu_X_var_b_Y -> Y
...
#                 print(f"  {combo_name}: κ_mu={data['kappa_mu']}, σ²_b={data['var_b']}")
    
#     # Auto-extract parameter values
#     kappa_mu_set = set()
#     var_b_set = set()
#     for combo_data in all_results.values():
#         kappa_mu_set.add(combo_data['kappa_mu'])
#         var_b_set.add(combo_data['var_b'])
    
#     kappa_mu_values = sorted(kappa_mu_set)
#     var_b_values = sorted(var_b_set)
    
#     if verbose:
#         print(f"Parameter grid: kappa_mu={kappa_mu_values}, var_b={var_b_values}")
...
#     combo_labels = []
#     for kappa_mu in kappa_mu_values:
#         for var_b in var_b_values:
#             combo_labels.append(f'κ={kappa_mu}, σ²={var_b}')
...
#                 for kappa_mu in kappa_mu_values:
#                     for var_b in var_b_values:
#                         values = metric_data.get((kappa_mu, var_b), [])
...
#             fig2, ax2 = plt.subplots(1, 1, figsize=(12, 8))  # 3:2 ratio
            
#             preservation_colors = ['#158945', '#4A4A4A']  # Light green + dark gray
...
#                 # Get consistent metric name and direction from metric_info
...
#     if diff_metrics:
#         fig3, ax3 = plt.subplots(1, 1, figsize=(12, 8))  # 3:2 ratio
        
#         diff_colors = ["#6778B6", "#A46F6F"]  # Light green + dark gray
...
#                 label = f"{metric_info[metric_name]['name']} {direction}"
#             else:
#                 # Fallback for metrics not in metric_info
...
# def plot_batch_comparison(clean_data, with_batch_data, combat_data, batch_labels, save_path=None):
#     """
#     Plot batch distributions for three datasets: clean, with batch effects, and ComBat corrected.
#     """
    
#     # Debug: Print shapes to understand the data structure
#     print(f"Clean data shape: {clean_data.shape}")
#     print(f"With batch data shape: {with_batch_data.shape}")
#     print(f"Combat data shape: {combat_data.shape}")
#     print(f"Batch labels shape: {batch_labels.shape}")
    
#     # Ensure data is in the right format (samples x features)
#     # If data is features x samples, transpose it
#     if clean_data.shape[0] < clean_data.shape[1]:
#         clean_data = clean_data.T
#         with_batch_data = with_batch_data.T
#         combat_data = combat_data.T
#         print("Transposed data to samples x features format")
    
#     # Make sure batch_labels matches the number of samples
#     n_samples = clean_data.shape[0]
#     if len(batch_labels) != n_samples:
#         print(f"Warning: batch_labels length ({len(batch_labels)}) != n_samples ({n_samples})")
#         # Use the minimum length to avoid index errors
#         min_len = min(len(batch_labels), n_samples)
#         clean_data = clean_data[:min_len]
#         with_batch_data = with_batch_data[:min_len]
#         combat_data = combat_data[:min_len]
...
#     datasets = [
#         ('Clean Data (no BE)', clean_data),
#         ('With Batch Effects', with_batch_data), 
#         ('ComBat Corrected', combat_data)
#     ]
...
#         for feature_idx in range(min(4, data.shape[1])):
#             all_values.extend(data[:, feature_idx])
    
#     y_min, y_max = np.min(all_values), np.max(all_values)
#     y_margin = (y_max - y_min) * 0.05
#     y_limits = [y_min - y_margin, y_max + y_margin]
    
#     # Plot first 4 features across the 3 datasets
#     for feature_idx in range(min(4, clean_data.shape[1])):  # Ensure we don't exceed available features
#         for dataset_idx, (title, data) in enumerate(datasets):
...
#                 if np.any(mask):  # Only add if there are samples in this batch
#                     batch_data.append(data[mask, feature_idx])
#                     batch_names.append(f'Batch {batch}')
            
#             # Create violin plot only if we have data
#             if batch_data:
#                 violin_parts = ax.violinplot(batch_data, positions=range(len(batch_names)), 
#                                            showmeans=True, showmedians=True)
                
#                 # Color the violins
#                 for pc, color in zip(violin_parts['bodies'], colors[:len(batch_data)]):
